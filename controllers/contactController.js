...
exports.submitContact = async function (req, res, next) {
  ...
  if (!validator.isEmail(req.body.email)) {
    console.log("invalid email detecter")
    return res.json({ message: "Sorry!" })
  }
  ...
  try {
    const promise1 = transport.sendMail({
      to: ourObject.email,
      from: "petproject@gmail.com",
      subject: `Thank u for ur intrest in ${doesPetExist.name}`,
      html: `<h3 style="color:purple; font-size: 30px; font-weight:normal;">Thank you!</h3>
      <p>We appreciate your interest in ${doesPetExist.name} and one of our staff members will reach out your shortly! Below is a copy of the message you sent us for your personal records:</p>
    ...
    const promise2 = transport.sendMail({
      ...
      from: "petproject@gmail.com",
      subject: `Someone is intrested in ${doesPetExist.name}`,
      html: `<h3 style="color:Red; font-size: 30px; font-weight:normal;">New Contact!</h3>
      <p>Name:${ourObject.name}<br>
      Pet Inerested In: ${doesPetExist}<br>
